...
const EmployeeManagement = ({ showNotify }) => {
    ...
    const handleAdd = async (e) => {
        ...
        try {
            ...
            fetchEmployees();
        } catch (err) { showNotify(err.response?.data?.message || "Add failed", "error"); }
    };
                            ...
                            <tr key={emp.id} style={styles.tr}>
                                <td style={styles.td}>
                                    <div style={styles.userCell}>
                                        ...
                                        <div><div style={{fontWeight:'700'}}>{emp.name}</div><div style={{fontSize:'0.8em', color:'#888'}}>{emp.employee_id}</div></div>
                                    </div>
                                </td>
                                ...
                                <td style={styles.td}>
                                    <div style={{position:'relative'}}>
                                        <button style={styles.dots} onClick={(e) => {e.stopPropagation(); setOpenDropdownId(openDropdownId === emp.id ? null : emp.id)}}>⋮</button>
                                        {openDropdownId === emp.id && (
                                            <div style={styles.dropdown}>
                                                <div style={styles.dropItem} onClick={() => {setFormData(emp); setIsEditOpen(true)}}>Edit</div>
                                                <div style={{...styles.dropItem, color:'red'}} onClick={() => setDeleteItem(emp)}>Delete</div>
                                            </div>
                                        )}
                                    </div>
                                </td>
                    ...
                    <div style={styles.modal}>
                        ...
                        <form onSubmit={isAddOpen ? handleAdd : handleUpdate}>
                            ...
                            <div style={styles.modalActions}>
                                <button type="button" style={styles.cancelBtn} onClick={() => {setIsAddOpen(false); setIsEditOpen(false); setFormData({name:'', email:'', dob:'', employee_id:''})}}>Cancel</button>
...
const styles = {
    ...
    dropdown: { position: 'absolute', right: 0, top: '30px', background: '#fff', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', borderRadius: '10px', zIndex: 10, width: '150px' },
    ...
    cancelBtn: { padding: '10px 20px', border: 'none', borderRadius: '8px', cursor: 'pointer', background:'#eee' },
